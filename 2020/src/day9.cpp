...
long long solve_day9_part1(const std::vector<long long>& input, const size_t preable_length) {
	bool found = false;
	for (size_t k = 0; k < input.size() - preable_length; ++k) {
		for (size_t i = k; i < k + (preable_length - 1)*(!found); ++i) {
			for (size_t j = i + 1; j < (k + preable_length)*(!found); ++j) {
				if (input[i] + input[j] == input[k + preable_length]) { found = true; }
			}
		}
		if (!found) { 
			return input[k + preable_length]; 
		}
		found = false;
	}
...
TEST_CASE("test day9 part 1") {
	size_t awnser = solve_day9_part1(
